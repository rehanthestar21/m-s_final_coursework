...
class OptionCalculator:
    ...
    def calculate_option_premiums(
        ...
        maturity_date, 
        interest_rate, 
        volatility_map,  # dictionary mapping "YYYY-MM" to volatility
        noise_level
    ):
        """
        Calculate call option premiums from a given CSV file of stock prices, using monthly volatilities.
        ...
        # Adjust indexing and columns if needed based on actual CSV format
        # Assuming the CSV has the standard Yahoo columns: Date, Open, High, Low, Close, Adj Close, Volume
        # If it differs, adjust accordingly. Here it's assumed the first row is header.
        # If your CSV differs, please adapt the code.
        ...
        for _, row in cleaned_data.iterrows():
            ...
            # Determine the volatility for the current month
            current_month = current_date.strftime("%Y-%m")
            sigma = volatility_map.get(current_month, 0.12)  # Default fallback if not found

            if time_to_maturity > 0:
                option_price = self.black_scholes_call(price, strike_price, time_to_maturity, interest_rate, sigma)
...
class Plotter:
    ...
    def plot_comparison(time_converted, last_price, results, random=True):
        """
        Plot comparison of real premium price (from Polygon) and Simulated Black-Scholes premium on the same y-axis.
        """
        plt.figure(figsize=(12, 6))
...
if __name__ == "__main__":
    ...
    # Load the monthly volatility data from CSV
    # Assuming the CSV has columns: Month, Average Volatility
    # For example, Month in the format "YYYY-MM" and a float volatility.
    monthly_vol_data_file = "Monthly_Volatility_Data_for_2023.csv"  # Update this with the actual CSV name
    monthly_vol_df = pd.read_csv(monthly_vol_data_file)
    # Make sure the CSV's Month column is in "YYYY-MM" format (or adapt parsing)
    # If it's already like "2023-01", etc., no change needed.
    # Create a dictionary for month to volatility mapping
    month_vol_map = dict(zip(monthly_vol_df['Month'].astype(str), monthly_vol_df['Average Volatility'].astype(float)))
    ...
    # Step 3: Calculate Option Premiums using Black-Scholes with monthly volatilities
    ...
    results, results_file = calculator.calculate_option_premiums(
        ...
        volatility_map=month_vol_map,  # Use the monthly volatility map
